...
class PatientRepository extends BaseRepository {
  ...
  Future<PatientProfile> getProfile(String userId) => safeQuery(() async {
    ...
    userData ??= await db
        .from('users')
        .insert({'id': userId, 'email': '', 'full_name': 'New User'})
        .select()
        .single();
...
// In patient_repository.dart, update the updateProfile method:
...
}) => safeVoidQuery(() async {  // Change from safeQuery to safeVoidQuery
  await Future.wait([
...
// Also update switchRole method:
Future<void> switchRole(String userId, String role) => safeVoidQuery(() async {
  await db
      .from('users')
      .update({'role': role})
      .eq('id', userId);

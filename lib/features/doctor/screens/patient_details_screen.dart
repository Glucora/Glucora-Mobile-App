...
class _PatientDetailsScreenState extends State<PatientDetailsScreen>
  ...
  // ── State ───────────────────────────────────────────────────────────────────
  PatientDetails? _details;
  bool _isLoading = true;

  /// Incremented after every successful fetch so the care-plan card's
  /// [ValueKey] changes and Flutter fully rebuilds it.
  int _carePlanReloadKey = 0;

  // ── Convenience getters (mirror the old computed properties) ────────────────

  Map<String, dynamic>? get _patientProfile => _details?.profile;
  Map<String, dynamic>? get _carePlan => _details?.carePlan;
  List<Map<String, dynamic>> get _glucoseReadings =>
      _details?.glucoseReadings ?? [];
  List<Map<String, dynamic>> get _insulinDoses => _details?.insulinDoses ?? [];
  List<Map<String, dynamic>> get _alerts => _details?.alerts ?? [];
  Map<String, dynamic>? get _device => _details?.device;
  Map<String, dynamic>? get _latestPrediction => _details?.latestPrediction;
  Map<String, dynamic>? get _latestIob => _details?.latestIob;
  ...
  double get _tddToday {
    ...
    return _insulinDoses
        .where((d) =>
            DateTime.parse(d['delivered_at']).toLocal().isAfter(startOfDay))
  ...
  String get _currentBasalRate {
    final basalDoses =
        _insulinDoses.where((d) => d['dose_type'] == 'Basal').toList();
  ...
  Future<void> _fetchPatientData() async {
    try {
      final details = await _repo.getPatientDetails(widget.patientId);
      if (!mounted) return;
      setState(() {
        _details = details;
        _carePlanReloadKey++;
  ...
  Future<void> _deletePatient() async {
    final doctorUserId = Supabase.instance.client.auth.currentUser!.id;
    await _repo.removePatient(
  ...
  AppBar _buildAppBar(BuildContext context) {
    final colors = context.colors;
    final formattedId =
        '#PT-${widget.patientId.toString().padLeft(5, '0')}';
    return AppBar(
          ...
          body: OrientationBuilder(
            builder: (ctx, orientation) => LocationView(
              patient: LocationPatientInfo(
                patientUserId:
                    _patientProfile!['user_id'] as String,
  ...
  Widget _buildPatientHeader(BuildContext context) {
    final colors = context.colors;
    final usersMap =
        _patientProfile?['users'] as Map<String, dynamic>?;
    final age =
    ...
    final latest =
        _glucoseReadings.isNotEmpty ? _glucoseReadings.last : null;
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    _headerChip(
                        Icons.bluetooth, 'CGM Online', Colors.greenAccent),
                    const SizedBox(width: 8),
                    _headerChip(Icons.water_drop_outlined, 'Pump Active',
  ...
  Widget _buildDeletePatientButton(BuildContext context) {
    ...
    return SizedBox(
      ...
      child: OutlinedButton.icon(
        ...
        label: const TranslatedText('Remove Patient',
            style:
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Row(
          children: [
            ...
            const SizedBox(width: 8),
            const TranslatedText('Remove Patient',
                style: TextStyle(
  ...
  Widget _buildGlucoseSummaryCards(BuildContext context) {
    ...
    return Row(
      children: [
        Expanded(
            child: _statCard(context, 'Average\nGlucose',
                avg.toStringAsFixed(0), 'mg/dL', colors.accent)),
        const SizedBox(width: 10),
        Expanded(
            child: _statCard(context, 'GMI\nEst. A1C',
                gmi.toStringAsFixed(1), '%', const Color(0xFF5B8CF5))),
        const SizedBox(width: 10),
        Expanded(
            child: _statCard(context, 'Glucose\nVariability',
                cv.toStringAsFixed(0), 'CV%', colors.warning)),
        const SizedBox(width: 10),
        Expanded(
            child: _statCard(context, 'Time\nIn Range', '$inRange', '%',
  ...
  Widget _buildTimeInRangeCard(BuildContext context) {
    ...
    int pct(bool Function(double) test) =>
        total == 0
            ? 0
            : (values.where(test).length / total * 100).round();
      ...
      child: Column(
        children: [
            ...
            children: [
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    _tirRow('Very High (>250)', veryHigh,
                        const Color(0xFFD32F2F)),
                    const SizedBox(height: 6),
                    _tirRow('High (181–250)', high,
                        const Color(0xFFFF9F40)),
                    const SizedBox(height: 6),
                    _tirRow('In Range (70–180)', inRange,
                        const Color(0xFF2BB6A3)),
                    const SizedBox(height: 6),
                    _tirRow(
                        'Low (54–69)', low, const Color(0xFFFBC02D)),
                    const SizedBox(height: 6),
                    _tirRow('Very Low (<54)', veryLow,
              ...
              SizedBox(
                ...
                height: 90,
                child: CustomPaint(
                  painter: TIRPieChartPainter(segments: [
                    TIRSegment(
                  ...
                  child: Center(
                    child: Column(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        TranslatedText('$inRange%',
                            style: const TextStyle(
                                fontSize: 16,
                                fontWeight: FontWeight.w800,
                                color: Color(0xFF2BB6A3))),
                        const TranslatedText('TIR',
                            style: TextStyle(
                                fontSize: 10,
                                color: Colors.grey)),
                      ],
          ...
          ClipRRect(
            ...
            child: Row(
              children: [
                _tirBar(veryHigh.toDouble(),
                    const Color(0xFFD32F2F)),
                _tirBar(
                    high.toDouble(), const Color(0xFFFF9F40)),
                _tirBar(inRange.toDouble(),
                    const Color(0xFF2BB6A3)),
                _tirBar(
                    low.toDouble(), const Color(0xFFFBC02D)),
                _tirBar(veryLow.toDouble(),
                    const Color(0xFFB71C1C)),
              ],
            ),
          ),
  ...
  Widget _buildPumpStatusCard(BuildContext context) {
    ...
    final batteryRaw =
        _device?['battery_health'] as String? ?? '—';
    final lastSyncAt = _device?['last_sync_at'] as String?;
    final lastSyncDisplay =
        lastSyncAt != null ? _timeAgo(lastSyncAt) : '—';
    ...
    final autoCount =
        insulinLog.where((d) => d.source == 'AID Auto').length;
      ...
      child: Column(
        children: [
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    TranslatedText(deviceName,
                        style: TextStyle(
                            fontWeight: FontWeight.w700,
                            fontSize: 15,
                            color: colors.textPrimary)),
                    TranslatedText('Last sync: $lastSyncDisplay',
                        style: TextStyle(
                            color: colors.textSecondary,
                            fontSize: 12)),
                  ],
              ...
              _statusBadge(context, isActive ? 'Active' : 'Inactive',
          ...
          Row(
            children: [
              Expanded(
                  child: _pumpStat('Battery', batteryRaw,
                      'Device battery', const Color(0xFF6FCF97))),
              const SizedBox(width: 10),
              Expanded(
                  child: _pumpStat('Auto Doses', '$autoCount',
                      'AID-delivered', const Color(0xFF5B8CF5))),
              const SizedBox(width: 10),
              Expanded(
                  child: _pumpStat('Basal Rate', _currentBasalRate,
                      'Last recorded', colors.warning)),
            ],
          ),
  ...
  Widget _buildAIDStatusCard(BuildContext context) {
    ...
    final predictedValue =
        _latestPrediction?['predicted_value_mg_dl'];
    ...
    final horizonDisplay = horizon != null
        ...
        : '—';
    final riskLevel =
        _latestPrediction?['risk_level'] as String? ?? '—';
    final modelVersion =
        _latestPrediction?['model_version'] as String? ?? '—';

    final isf =
        _carePlan?['insulin_sensitivity_factor'] as String? ?? '—';
    final carbRatio = _carePlan?['carb_ratio'];
    final carbRatioDisplay = carbRatio != null
        ? '1 U : ${(carbRatio as num).toStringAsFixed(0)}g carbs'
        : '—';
    final aidEnabled =
        _carePlan?['aid_mode_enabled'] as bool? ?? false;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
            ...
            children: [
              Container(
                ...
                child: const Icon(Icons.auto_awesome,
                    color: Color(0xFF5B8CF5), size: 22),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const TranslatedText('AI-Powered AID System',
                        style: TextStyle(
                            fontWeight: FontWeight.w700,
                            fontSize: 15)),
                    TranslatedText(
          ...
          _aidRow(Icons.show_chart,
              'Predicted Glucose ($horizonDisplay)',
              predictedDisplay, colors.textPrimary),
          const Divider(height: 1, color: Color(0xFFF0F0F0)),
          _aidRow(Icons.trending_up, 'Current Glucose Trend',
              trendDisplay, colors.accent),
          const Divider(height: 1, color: Color(0xFFF0F0F0)),
          _aidRow(Icons.warning_amber_rounded, 'Risk Level',
              riskLevel.toUpperCase(), riskColor),
          const Divider(height: 1, color: Color(0xFFF0F0F0)),
          _aidRow(
              Icons.tune,
              ...
              Colors.blueGrey),
          const Divider(height: 1, color: Color(0xFFF0F0F0)),
          _aidRow(Icons.rice_bowl_outlined, 'Insulin-to-Carb Ratio',
              carbRatioDisplay, Colors.blueGrey),
  ...
  Widget _aidRow(IconData icon, String label, String value,
      Color valueColor) {
    return Padding(
  ...
  Widget _buildAlertsCard(BuildContext context) {
    if (_alerts.isEmpty) {
      return Container(
        ...
        decoration: _cardDecoration(context),
        child: const Center(
          child: TranslatedText('No recent alerts',
    ...
    return Container(
      ...
      child: Column(
        children: _alerts.asMap().entries.map((entry) {
          ...
          final title =
              alert['title'] as String? ?? type.toUpperCase();
          final message = alert['message'] as String? ?? '';
          final dt =
              DateTime.parse(alert['triggered_at']).toLocal();
  ...
  Widget _alertRow(IconData icon, String tag, String message,
  ...
  Widget _buildCarePlanCard(BuildContext context) {
    ...
    final targetMin =
        plan?['target_glucose_min']?.toString() ?? '70';
    final targetMax =
        plan?['target_glucose_max']?.toString() ?? '180';
    final insulinType = plan?['insulin_type'] ?? '—';
    final maxDose =
        plan?['max_auto_dose_units']?.toString() ?? '—';
    ...
    return Container(
      ...
      child: Column(
        children: [
          _carePlanRow(
              'Target Range', '$targetMin – $targetMax mg/dL', context),
          const Divider(height: 1, color: Color(0xFFF5F5F5)),
          _carePlanRow('Insulin Type', insulinType, context),
          const Divider(height: 1, color: Color(0xFFF5F5F5)),
          _carePlanRow(
              'Max Auto-Bolus', '$maxDose U per event', context),
          const Divider(height: 1, color: Color(0xFFF5F5F5)),
          _carePlanRow('Notes', notes, context),
          const Divider(height: 1, color: Color(0xFFF5F5F5)),
          _carePlanRow(
              'Next Appointment', apptDisplay, context),
          const SizedBox(height: 12),
          SizedBox(
            width: double.infinity,
            height: 46,
            child: ElevatedButton.icon(
              ...
              onPressed: () async {
                ...
                if (updated == true) await _fetchPatientData();
              },
              icon: const Icon(Icons.edit_outlined,
                  size: 16, color: Colors.white),
              label: const TranslatedText('Edit Care Plan',
                  style: TextStyle(
                      color: Colors.white,
                      fontWeight: FontWeight.w700)),
            ),
        ...
        children: [
              ...
              children: [
                ...
                SizedBox(
                  ...
                  child: glucoseHistory.length < 2
                      ? const Center(
                          child: TranslatedText('Not enough data',
                              style: TextStyle(
                                  color: Colors.grey)))
                      : CustomPaint(
          ...
          Container(
            ...
            child: Column(
              children: [
                _cgmRow('Device',
                    _device?['device_name'] ?? '—', context),
                _cgmRow('Type',
                    _device?['device_type'] ?? '—', context),
                _cgmRow('Firmware',
                    _device?['firmware_version'] ?? '—', context),
                _cgmRow('Battery',
                    _device?['battery_health'] ?? '—', context),
                _cgmRow(
                    'Last Sync',
                    _device?['last_sync_at'] != null
                        ? _timeAgo(
                            _device!['last_sync_at'] as String)
                        : '—',
                    context),
                _cgmRow(
                    'Status',
  ...
  Widget _readingListTile(
      GlucoseReading reading, BuildContext context) {
    final colors = context.colors;
    Color valueColor;
    String status;
    ...
    return ListTile(
      ...
      leading: Container(
        ...
        child: Center(
          child: TranslatedText('${reading.value}',
              style: TextStyle(
                  fontWeight: FontWeight.w800,
                  color: valueColor,
                  fontSize: 13)),
      ...
      title: TranslatedText('${reading.time}  •  $status',
          style: TextStyle(
              fontSize: 13,
              fontWeight: FontWeight.w600,
              color: colors.textPrimary)),
      subtitle: reading.mealTag != null
          ? Row(children: [
              ...
              const SizedBox(width: 4),
              TranslatedText(reading.mealTag!,
                  style: const TextStyle(
                      fontSize: 11, color: Colors.grey)),
            ])
          : const TranslatedText('CGM reading',
              style: TextStyle(
                  fontSize: 11, color: Colors.grey)),
      trailing: TranslatedText('mg/dL',
          style: TextStyle(
            ...
            child: insulinLog.isEmpty
                ...
                : ListView.separated(
                    ...
                    itemBuilder: (context, i) =>
                        _insulinLogTile(
                            insulinLog[i], context),
                  ),
          ),
          const SizedBox(height: 36),
          _sectionTitle(
              context, 'Active Insulin on Board (IOB)'),
          const SizedBox(height: 12),
            ...
            children: [
              ...
              if (diaDisplay != '—')
                Container(
                  ...
                  child: TranslatedText('DIA: $diaDisplay',
                      style: const TextStyle(
                          fontSize: 12,
                          fontWeight: FontWeight.w700,
                          color: Color(0xFF5B8CF5))),
  ...
  Widget _insulinLogTile(
      InsulinDose dose, BuildContext context) {
    final colors = context.colors;
    Color typeColor;
    IconData typeIcon;
    switch (dose.type) {
    ...
    return ListTile(
      ...
      title: Row(
        children: [
          TranslatedText('${dose.units} U  •  ${dose.type}',
              style: TextStyle(
                  fontSize: 13,
                  fontWeight: FontWeight.w700,
                  color: colors.textPrimary)),
          ...
          Container(
            ...
            child: TranslatedText(dose.source,
                style: TextStyle(
                    fontSize: 9,
                    fontWeight: FontWeight.w700,
                    color: dose.source == 'AID Auto'
      ...
      subtitle: TranslatedText('${dose.time}  •  ${dose.reason}',
          style: TextStyle(
              fontSize: 12, color: colors.textSecondary)),
    );
  }
  ...
  String _timeAgo(String isoString) {
    final diff = DateTime.now()
        .difference(DateTime.parse(isoString).toLocal());

...
class GuardianConnectionRequest {
  final String id;
  ...
  GuardianConnectionRequest({
    required this.id,
    required this.patientName,
    required this.sentAgo,
    required this.avatarInitials,
    required this.requestedBy,
    this.status = GuardianRequestStatus.pending,
  });
}
...
class _GuardianRequestsScreenState extends State<GuardianRequestsScreen>
  ...
  List<GuardianConnectionRequest> _requests = [];

  List<GuardianConnectionRequest> get _incoming => _requests
      .where((r) => r.requestedBy == 'patient' && r.status == GuardianRequestStatus.pending)
      .toList();

  List<GuardianConnectionRequest> get _sent => _requests
      .where((r) => r.requestedBy == 'guardian' && r.status == GuardianRequestStatus.pending)
      .toList();

  List<GuardianConnectionRequest> get _declined =>
      _requests.where((r) => r.status == GuardianRequestStatus.declined).toList();
  ...
  Future<void> _fetchRequests() async {
    try {
      ...
      final guardianRow = await supabase
          ...
          .eq('user_id', userId)
          .single();
      final guardianId = guardianRow['id'] as String;

      final response = await supabase
          .from('guardian_requests_view')
          .select()
      ...
      print('Error fetching guardian requests: $e');
    }
  }
  ...
  Future<void> _sendRequest({
    ...
    setState(() {
      _requests.add(GuardianConnectionRequest(
  ...
  void _openSearchSheet() {
    showModalBottomSheet(
      ...
      builder: (_) => _GuardianSearchPatientSheet(
  ...
  void _accept(GuardianConnectionRequest request) async {
    try {
      await supabase
          .from('guardian_patient_connections')
          .update({
      ...
      if (!mounted) return;
      setState(() => request.status = GuardianRequestStatus.accepted);
      _showSnackbar('${request.patientName} accepted');
    } catch (e) {
      print('Exception accepting: $e');
      _showSnackbar('Failed to accept request');
    }
  }

  void _decline(GuardianConnectionRequest request) async {
    try {
      await supabase
          .from('guardian_patient_connections')
          .update({
      ...
      if (!mounted) return;
      setState(() => request.status = GuardianRequestStatus.declined);
      _showSnackbar('${request.patientName} declined');
    } catch (e) {
      print('Exception declining: $e');
      _showSnackbar('Failed to decline request');
    }
  }

  void _withdraw(GuardianConnectionRequest request) async {
    await supabase
        .from('guardian_patient_connections')
        .delete()
        .eq('id', int.parse(request.id));
          ...
          return Column(children: [
            ...
            Expanded(
              child: TabBarView(
                ...
                children: [
                  _buildRequestList(context, _incoming, tabType: 'incoming', isLandscape: isLandscape),
                  _buildRequestList(context, _sent, tabType: 'sent', isLandscape: isLandscape),
                  _buildRequestList(context, _declined, tabType: 'declined', isLandscape: isLandscape),
                ],
              ),
            ),
          ]);
      ...
      child: isLandscape
          ? Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('Connection Requests', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: colors.textPrimary)),
                Text('${_incoming.length} pending request${_incoming.length == 1 ? '' : 's'}',
                    style: TextStyle(fontSize: 13, color: colors.textSecondary)),
              ])),
              const SizedBox(width: 16),
              Row(children: [
                _summaryChip(context, 'Incoming', _incoming.length, colors.accent),
          ...
          : Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('Connection Requests', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: colors.textPrimary)),
          ...
          Tab(child: Row(mainAxisSize: MainAxisSize.min, children: [
            ...
            if (_incoming.isNotEmpty) ...[
              ...
              Container(
                ...
                child: Text('${_incoming.length}', style: const TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.w800)),
              ),
            ],
          ])),
          const Tab(text: 'Sent'),
  ...
  Widget _buildRequestList(BuildContext context, List<GuardianConnectionRequest> requests,
      {required String tabType, required bool isLandscape}) {
    final colors = context.colors;
    if (requests.isEmpty) {
      return Center(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Text(
            tabType == 'incoming' ? 'No incoming requests'
                : tabType == 'sent' ? 'No sent requests'
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) => _GuardianRequestCard(
...
class _GuardianRequestCard extends StatelessWidget {
  final GuardianConnectionRequest request;
  final String tabType;
  final VoidCallback onAccept;
  final VoidCallback onDecline;
  final VoidCallback onWithdraw;

  const _GuardianRequestCard({
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text(request.sentAgo, style: TextStyle(fontSize: 12, color: colors.textSecondary)),
          ])),
        ]),
        if (tabType == 'incoming') ...[
...
class _GuardianSearchPatientSheet extends StatefulWidget {
  final Future<void> Function({required int patientProfileId, required String patientName}) onSendRequest;
  final VoidCallback onRequestChanged;

  const _GuardianSearchPatientSheet({
    required this.onSendRequest,
    required this.onRequestChanged,
  });

  @override
  State<_GuardianSearchPatientSheet> createState() => _GuardianSearchPatientSheetState();
}

class _GuardianSearchPatientSheetState extends State<_GuardianSearchPatientSheet> {
  final _phoneController = TextEditingController();
  bool _isSearching = false;
  bool _isSending = false;
  List<Map<String, dynamic>> _results = [];
  String? _errorMessage;
  String? _guardianProfileId;

  @override
  void initState() {
    super.initState();
    _loadGuardianProfileId();
  }

  Future<void> _loadGuardianProfileId() async {
    final userId = supabase.auth.currentUser!.id;
    final row = await supabase
        ...
        .eq('user_id', userId)
        .single();
    setState(() => _guardianProfileId = row['id'] as String);
  }

  Future<void> _search() async {
    final phone = _phoneController.text.trim();
    if (phone.isEmpty || _guardianProfileId == null) return;

    setState(() { _isSearching = true; _results = []; _errorMessage = null; });
    ...
    try {
      ...
      if ((userRows as List).isEmpty) {
        setState(() { _errorMessage = 'No patient found with this phone number.'; _isSearching = false; });
        return;
      }
      ...
      for (final user in userRows) {
        ...
        final myConnection = await supabase
            .from('guardian_patient_connections')
            .select('id, status')
            .eq('guardian_id', _guardianProfileId!)
        ...
        found.add({
          ...
          'connectionId': (myConnection as List).isNotEmpty ? myConnection[0]['id'].toString() : null,
      ...
      setState(() { _errorMessage = 'Something went wrong. Please try again.'; _isSearching = false; });
    }
  }
  ...
  Future<void> _add(Map<String, dynamic> patient) async {
    ...
    try {
      ...
      setState(() { _errorMessage = 'Failed to send request.'; _isSending = false; });
    }
  }
  ...
  Future<void> _withdraw(Map<String, dynamic> patient) async {
    ...
    try {
      ...
      setState(() { _errorMessage = 'Failed to withdraw request.'; _isSending = false; });
    }
  }
    ...
    switch (status) {
      case 'accepted':
        return Container(
          ...
          child: const Text('Already connected', style: TextStyle(color: Colors.green, fontSize: 12, fontWeight: FontWeight.w600)),
        );
      case 'pending':
        return Row(mainAxisSize: MainAxisSize.min, children: [
          Container(
            ...
            child: const Text('Request pending', style: TextStyle(color: Colors.orange, fontSize: 12, fontWeight: FontWeight.w600)),
          ),
          const SizedBox(width: 8),
          GestureDetector(
            ...
            child: Container(
              ...
              child: Text('Withdraw', style: TextStyle(color: colors.error, fontSize: 12, fontWeight: FontWeight.w600)),
            ),
          ),
        ]);
      case 'declined':
        return ElevatedButton(
          ...
          style: ElevatedButton.styleFrom(backgroundColor: colors.accent, foregroundColor: Colors.white, elevation: 0,
              padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10),
              shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))),
          child: const Text('Send Again', style: TextStyle(fontWeight: FontWeight.w700)),
        );
      default:
        return ElevatedButton(
          onPressed: _isSending ? null : () => _add(patient),
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Text('Find a Patient', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: colors.textPrimary)),
        const SizedBox(height: 4),
        Text('Search by phone number to send a connection request.',
            style: TextStyle(fontSize: 13, color: colors.textSecondary)),
        const SizedBox(height: 20),
        Row(children: [
          Expanded(
            child: TextField(
              ...
              decoration: InputDecoration(
                ...
                enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12),
                    borderSide: BorderSide(color: colors.textSecondary.withValues(alpha: 0.15))),
        ...
        if (_errorMessage != null) ...[
          const SizedBox(height: 14),
          Text(_errorMessage!, style: TextStyle(color: colors.error, fontSize: 13)),
        ],
        if (_results.isNotEmpty) ...[
          const SizedBox(height: 16),
          ..._results.map((patient) => Container(
            margin: const EdgeInsets.only(bottom: 10),
            padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 12),
            decoration: BoxDecoration(
              color: colors.background, borderRadius: BorderRadius.circular(12),
              border: Border.all(color: colors.textSecondary.withValues(alpha: 0.15)),
            ),
            child: Row(children: [
              CircleAvatar(radius: 22, backgroundColor: colors.accent.withValues(alpha: 0.15),
                  child: Text(_initials(patient['full_name']),
                      style: TextStyle(color: colors.primaryDark, fontWeight: FontWeight.bold, fontSize: 13))),
              const SizedBox(width: 12),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(patient['full_name'], style: TextStyle(fontWeight: FontWeight.w700, fontSize: 14, color: colors.textPrimary)),
                const SizedBox(height: 2),
                Text(patient['phone_no'], style: TextStyle(fontSize: 12, color: colors.textSecondary)),
              ])),
